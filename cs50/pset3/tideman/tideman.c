...
void record_preferences(int ranks[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        int pref_index = ranks[i];
        preferences[pref_index][pref_index] = 0;
        for (int j = i + 1; j < candidate_count; j++)
        {
            int less_index = ranks[j];
            preferences[pref_index][less_index]++;
        }
    }
    return;
}
...
void sort_pairs(void)
{
    ...
    for (int i = 0; i < pair_count; i++)
    {
        ...
        {
            int curr_index = j;
            int curr_strength = preferences[pairs[j].winner][pairs[j].loser] - preferences[pairs[j].loser][pairs[j].winner];
...
bool is_cycle_r(int winner, int loser)
{
    if (winner == loser)
    {
        return true;
